...
class Maze {
 public:
   ...
   * @throw std::invalid_argument If the file path is incorrect or the maze size
   * is invalid.
   */
   ...
   *
   * Saves the maze to a file with a unique name, including the maze size and a
   * timestamp.
   */
   ...
   *
   * Finds a solution to the maze from the specified starting and ending
   * coordinates.
   *
   ...
   * @throw std::logic_error If the beginning and ending coordinates are the
   * same.
   */
   ...
   *
   * Returns a reference to a maze ceil at the specified row and column
   * coordinates.
   *
   * @param row The row coordinate.
   * @param col The column coordinate.
   * @return A reference to the maze ceil.
   * @throw std::out_of_range If the row or column coordinates are out of
   * bounds.
   */
 ...
 private:
   ...
   *
   * This method is used internally to perform a wave search in the maze
   * structure for finding a solution.
   *
   ...
   *
   * Finds the shortest way in the maze based on a previously performed wave
   * search.
   *
   ...
   *
   * Finds the next maze ceil in the shortest path based on the current
   * coordinates and step information.
   *
 ...
 private:
  std::vector<Ceil>
      ceils_; /**< The vector of maze ceils representing the maze structure. */
};
}  // namespace maze
