...
std::vector<Ceil> Eller::GeneratePerfectMaze(int size) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    for (int j = 0; j < size; ++j) {
      ...
      if (j == size - 1 || current[j] == current[j + 1] || RandBool()) {
        ...
        std::replace_if(current.begin(), current.end(), [=](int value) {
            return value == current[j + 1];
            },
            current[j]);
      }

      // 4. bottom walls creation
      if (i == size - 1 || (CheckBottomWallRule(current, perfect_maze, i, j) && RandBool())) {
  ...
  for (int j = 0; j < size - 1; ++j) {
    if (prev[j + 1] != prev[j]) {
      ...
      std::replace_if(prev.begin(), prev.end(), [=](int value) {
          return value == prev[j + 1];
          },
          prev[j]);
    }
  }
