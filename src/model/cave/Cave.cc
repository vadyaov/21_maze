...
std::size_t Cave::Size() const noexcept { return std::sqrt(ceils_.size()); }

bool Cave::At(int row, int col) const { return ceils_.at(row * Size() + col); }
...
void Cave::NextGeneration(int life_lim, int death_lim) {
  ...
  for (std::size_t i = 0; i < Size(); ++i)
    for (std::size_t j = 0; j < Size(); ++j) {
      ...
      if (At(i, j) == false) {
        if (alive_num <= death_lim) next[i * Size() + j] = true;
      } else {
        if (alive_num > life_lim + 1) next[i * Size() + j] = false;
      }
    }
